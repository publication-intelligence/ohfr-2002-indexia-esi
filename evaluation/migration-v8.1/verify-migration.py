...
def verify():
    receipt = read(HERE / 'original-v8-preservation.json')
    archive = HERE / receipt['archive_path']
    ...
    with zipfile.ZipFile(archive) as z:
        ...
        for key in ('source_scope', 'audience', 'audit_design', 'stage_application', 'deviations'):
        ...
        for before, after in zip(old_calc['dimensions'], calc['dimensions'], strict=True):
            ...
            assert before['pre_cap_percentage'] == after['pre_cap_percentage']
            for old_component, component in zip(before['components'], after['components'], strict=True):
                if component['component_id'] == 'high_value_treatment_recall_safeguard':
                    assert old_component['weight'] == old_component['effective_weight'] == 'cap_only'
                    assert component['weight'] == component['effective_weight'] == 'reported_diagnostic_only'
                    old_component = dict(old_component, weight='reported_diagnostic_only', effective_weight='reported_diagnostic_only')
                assert old_component == component, component['component_id']
    ...
    for gate in metadata['critical_gates']:
        if gate['triggered']:
            assert all(gate[key] for key in ('affected_evidence_ids', 'consequence_evidence', 'threshold_reason'))
    ...
    result = {'ok': True, 'registered_artifacts': len(state['artifacts']), 'changed_registered_artifacts': changed,
              ...
              'all_ordinary_components_unchanged': True, 'all_item_assessments_unchanged': True,
